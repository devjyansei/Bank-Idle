...
public class BuyManager : MonoBehaviour
{
    ...
    public GameObject securityWorkerPrefab;
    public Transform securityWorkerSpawnPoint;
    int boughtSecurityWorker;
    ...
    public void OnBuySecurityWorker()
    {
        if (MoneyBag.Instance.moneyOnPlayer >= WorkerCostData.Instance.currentSecurityWorkerCost && boughtSecurityWorker < Variables.Instance.securityWorkerBuyLimit)
        {
            for (int i = 0; i < WorkerCostData.Instance.currentSecurityWorkerCost; i++)
            {
                MoneyBag.Instance.DecraseMoney(1);
            }

            GameObject tempWorker = Instantiate(securityWorkerPrefab);
            tempWorker.transform.position = securityWorkerSpawnPoint.position;
            //WorkerCostData.Instance.currentTellerWorkerCost += WorkerCostData.Instance.tellerWorkerCostIncreaseValue;
            boughtSecurityWorker++;
